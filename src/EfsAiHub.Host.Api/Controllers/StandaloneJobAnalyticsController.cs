...
/// <summary>
/// Endpoints da saúde da fila standalone (<c>aihub.background_response_jobs</c>)
/// escopados por projeto. Mostra throughput, success rate, retries, latência
/// fila→running e workflow→completed, breakdown por workflow. Authorization
/// idêntica ao <see cref="ToolAnalyticsController"/>.
/// </summary>
[ApiController]
[Route("api/aihub/analytics/standalone-jobs")]
[Produces("application/json")]
public sealed class StandaloneJobAnalyticsController : ControllerBase
{
    private readonly IStandaloneJobAnalyticsRepository _repo;
    ...
    public StandaloneJobAnalyticsController(
        IStandaloneJobAnalyticsRepository repo,
    ...
    [HttpGet("summary")]
    [SwaggerOperation(Summary = "Resumo de uso/saúde da fila standalone no período (default: mês corrente). " +
                                "Retorna agregado (status counts, success/retry rate, p95 queue/total) + breakdown por workflow.")]
    [ProducesResponseType(typeof(StandaloneJobOverview), StatusCodes.Status200OK)]
    ...
    [HttpGet("timeseries")]
    [SwaggerOperation(Summary = "Série temporal de jobs criados + sucesso/falha por bucket (day|hour) + p95 fila→running.")]
    [ProducesResponseType(typeof(IReadOnlyList<StandaloneJobTimeseriesBucket>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
