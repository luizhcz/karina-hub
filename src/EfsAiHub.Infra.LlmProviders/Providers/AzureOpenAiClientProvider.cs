...
/// Suporta autenticação via TokenCredential ou ApiKey resolvida via <see cref="IRuntimeSecretStore"/>.
/// </summary>
public class AzureOpenAiClientProvider : ILlmClientProvider
{
    ...
    public AzureOpenAiClientProvider(IOptions<AzureAIOptions> options, TokenCredential credential, IRuntimeSecretStore secrets)
    {
        ...
        AgentDefinition definition, ChatClientAgentOptions options, CancellationToken ct = default)
    {
        ...
        return Task.FromResult<object>(chatClient.AsAIAgent(options));
    }
