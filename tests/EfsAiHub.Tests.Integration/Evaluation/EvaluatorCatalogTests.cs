...
public sealed class EvaluatorCatalogTests(IntegrationWebApplicationFactory factory)
{
    ...
    {
        var response = await _client.GetAsync("/api/aihub/evaluator-config/catalog");
    ...
    {
        var entries = await _client.GetFromJsonAsync<List<CatalogEntryDto>>("/api/aihub/evaluator-config/catalog");
